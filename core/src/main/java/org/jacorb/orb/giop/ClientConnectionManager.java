    ...
    implements Configurable
{
    ...
    public void releaseConnection( ClientConnection connection )
    {
        ...
        synchronized (connections) {
            if (connection.decClients() || connection.isClosed() || GIOPConnection.COMM_ERR_MANUAL.get()) {
                logger.warn("Waiting for release...");
                int sleep = GIOPConnection.CONN_RELEASE_DELAY.get();
